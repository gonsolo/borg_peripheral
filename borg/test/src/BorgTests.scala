  ...
  val tests = Tests {
    utest.test("borg_float_add") {
      simulate(new Borg) { borg =>
        ...
        val a = 5.0f
        val a_int = java.lang.Float.floatToIntBits(a)
        borg.io.data_in.poke(a_int.U)
        //borg.clock.step(1)
        val b_big_int = borg.io.data_out.peek().litValue
        val b = java.lang.Float.intBitsToFloat(b_big_int.toInt)
        println(s"The value of b is: $b")
        utest.assert(b == a)
